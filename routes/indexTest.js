...
router.get("/bundles", passport.authenticate('basic', {
    ...
    const {error} = validator.validatePackageQuery(getReqData(req));
    if (error) {
        return res.json({
            status: 2,
...
router.get("/transactions", passport.authenticate('basic', {
    ...
    let {accountId, begin_date, end_date, channel, maxRecords} = getReqData(req);
...
router.get("/balance", passport.authenticate('basic', {
    ...
    let {accountId, channel} = getReqData(req);
...
function getBundleDesc(name) {
    switch (name){
        case "All Weather":
            return "This bundle will switch you to the All Weather service class and cancel existing subscriptions"
        case "AlwaysON Group":
            return "This bundle will switch you to the AlwaysON Group service class and cancel existing subscriptions"
        case "AlwaysON":
            return "This bundle will switch you to the AlwaysON Business service class and cancel existing subscriptions"
        default:
            return  null
    }
